...
* Buyer is now free to set the desired values under each of the filters provides ( Price, Mileage, Make, Model etc.,). In addition to this high level filters, an attempt is make to filter even through the optional Meta Data that might be present on each of the Vehicle like (Body Type, Color and Trasmission etc.,) using a more dynamic query, but it is left unimplmented by the time this read me is written. Will surely be a future Enhancement.
